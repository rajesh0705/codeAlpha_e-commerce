// js/product.js
// Products page logic: render, search, filter, sort, actions (add/view)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Demo products (replace with fetch to your API if needed)
  const products = [
  ...
  // State from URL (optional prefilter)
      ...
      <article class="product-card">
        ...
        <div class="product-body">
          ...
          <div class="product-meta">
            ...
            <span style="color:#94a3b8; font-size:.85rem;">Pop • ${p.popularity}</span>
          </div>
  ...
  function updateResultsInfo(total, filtered) {
    ...
    $resultsInfo.textContent = filtered === total ? `${filtered} products` : `${filtered} of ${total} products`;
  }
  ...
  function applyFilters() {
    ...
    if (state.category !== 'all') {
      list = list.filter(p => p.category.toLowerCase() === String(state.category).toLowerCase());
    }
  ...
  $container?.addEventListener('click', (e) => {
    ...
    if (btn.dataset.role === 'add') {
      window.ProductMain?.addToCart(product, 1);
  ...
  applyFilters();
  window.ProductMain?.updateCartCount?.();
});